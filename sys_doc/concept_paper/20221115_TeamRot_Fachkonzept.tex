...
In Abschnitt~\ref{sec:anforderungen} werden die Anforderungen in Form von User Stories beschrieben. Zum Schluss wird auf die zu verwendenden Technologien zur Umsetzung des Projekts eingegangen, welche in Abschnitt~\ref{sec:methoden} näher beschrieben werden.
...
%Jedoch sind die Daten, welche IGDB bereit stellt nicht für die LOD-Cloud aufbereitet. Des Weiteren ist die Oberfläche der Website sehr klassisch aufgebaut, wodurch kein weiteres Interesse geweckt wird. Hier setzt das Projekt von Team Rot an.
Wie die Visualisierung eines Graphen möglicherweise aussehen kann, demonstriert die Anwendung Obsidian. Diese erstellt aus von Nutzern kreierten Markdown Dateien eine Graphenoberfläche. Über Verlinkungen zwischen den Markdown Dateien wächst der Graph an. Der Nutzer kann den Graph filtern oder gewisse Teile daraus hervorheben.

\begin{figure}[thpb]
    \centering
    \includegraphics[scale=0.17]{obsidian}
    \caption{Obsidian Graphenoberfläche~\cite{obsidian}}
    \label{fig:obsidian}
\end{figure}
...
\begin{itemize}
    ...
    \item Schwebt man mit der Maus über einen Konten öffnet sich ein Feld mit weiteren Informationen. 
    \item Im Informationsfeld wird angezeigt:
    \begin{itemize}
      \item Titel des Videospiels
      \item Genre
      \item Bewertung
    \end{itemize}
    \item Schwebt man mit der Maus über einen Knoten werden alle verbunden Knotenpunkte und Kanten im Graphen hervorgehoben.
\end{itemize}


\subsection{Suche per Texteingabe}

Als Benutzer möchte ich ein Videospiel per Texteingabe suchen können, da bei einem Graphen mit vielen Knoten und Kanten ein spezielles Spiel nur schlecht gefunden werden kann. Akzeptanzkriterien sind:
\begin{itemize}
...
\begin{itemize}
    ...
    \item Details die angezeigt werden:
    \begin{itemize}
    	\item Titel
    	\item Erscheinungsdatum
    	\item Beschreibung des Videospiels
    	\item Bewertung
    	\item Genre
...
\begin{itemize}
	\item Schaltfläche für Filter ist vorhanden.
    \item Filteroptionen sind: 
    \begin{itemize}
      \item Genre
      \item Plattform
      \item Hersteller
    ...
    \item Wird der Filter aufgehoben sollen alle entfernten Konten wieder angezeigt werden.
\end{itemize}
...
\begin{itemize}
    ...
    \item (Optional) Die Anwendung ist in der Cloud und prinzipiell erreichbar.
\end{itemize}
