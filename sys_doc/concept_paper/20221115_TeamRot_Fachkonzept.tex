...
Die Anwendung Obsidian erstellt aus von Nutzern kreierten Markdown Dateien eine Graphenoberfläche. Über Verlinkungen zwischen den Markdown Dateien wächst der Graph an. Der Nutzer kann den Graph filtern oder gewisse Teile daraus hervorheben. Diese Anwendung zeigt hervorragend, wie eine Visualisierung eines Graphen möglicherweise aussehen könnte.
Die Website igdb.com gibt einen Eindruck, was der Nutzer von einer Spielesuche erwarten kann. Der Funktionsumfang und die Spieleanzahl ist groß, sodass das Suchergebnis meist zufriedenstellend ist. Des Weiteren besitzt die Website eine gut dokumentierte Entwickler-API. Dies ermöglicht es Entwickler, Daten in Form von JSON, REST oder ProtoBuf zu erhalten. Jedoch sind die Daten, welche IGDB bereit stellt nicht für die LOD-Cloud aufbereitet. Des Weiteren ist die Oberfläche der Website sehr klassisch aufgebaut, wodurch kein weiteres Interesse geweckt wird. Hier setzt das Projekt von Team Rot an. Es wird eine Web-Applikation entworfen, welche die Daten anreichert, damit diese in die LOD-Cloud überführt werden können. Zusätzlich wird eine Graphenoberfläche entworfen, welche dazu einlädt, artverwandte Spiele auch ohne Filteroptionen zu entdecken.
...
\begin{itemize}
\item Graphenoberfläche soll direkt sichtbar sein beim besuchen der Website
\item Der Graph soll mit ausreichend vielen Knoten angereichert sein
\item Die Oberfläche soll visuell ansprechend sein und zum stöbern einladen
\end{itemize}

\subsection{Knotendetails}
Als Benutzer möchte ich mit einem Klick auf einem Knoten im Graphen weitere Details zu den im Knoten enthaltenen Spiel sehen. Somit erhalte ich ohne die Graphenoberfläche zu verlassen die wichtigsten Informationen zu einen Spiel. Akzeptanzkriterien sind:
\begin{itemize}
\item Knoten im Graphen sollen anklickbar sein
\item Der geklickte Knoten öffnet ein Feld mit weiteren Informationen zu den ausgewählten Knoten
\item Die wichtigsten Informationen befinden sich zusammengefasst im geöffneten Feld
\end{itemize}
...
\item Ergebnis sollte gut erkennbar hervor gehoben werden
...
Als Benutzer möchte ich eine Detailseite zu einem jeweilige Spiel, welche über den Knoten im Graphen erreicht wird. Somit muss ich als Nutzer nicht externe Internetseiten für weitere Informationen besuchen. Akzeptanzkriterien sind:
\begin{itemize}
\item Alle Details eines Spiels werden aufgelistet
\item Spiel wird vorgestellt
\end{itemize}

\subsection{(Optional) Suche}
Als Benutzer möchte ich ein Spiel suchen können, da bei einem größeren Graphen nur schwierig ein spezielles Spiel gefunden werden kann. Akzeptanzkriterien sind:
\begin{itemize}
\item Eingabefeld für Suche
\item Weiterleitung direkt zum Knotenpunkt, welcher das gewünschte Spiel beinhaltet
\item Grafische hervorherbung des Knotenpunkts
\end{itemize}
