...
Das Backend ist mit der Programmiersprache Python realisiert. Es dient als Schnittstelle zwischen Datenbank und Frontend. Zusätzlich ist es für die Verarbeitung von Ressourcen zuständig. % Ressourcen genauer beschreiben
Das Frontend kommuniziert mit dem Backend über eine RESTful-API, die mithilfe des Frameworks „FastAPI“~\cite{fastapi} implementiert ist.

Folgende Daten stellt die API für das Frontend zur Verfügung: %besser wäre hier einen Tabelle zur Beschreibung der API-Endpunkte
\begin{itemize}
    \item Daten für einen Graphen mit einem Root-Knoten, welcher bei der Startpage angezeigt wird
    \item Daten, welche Spielinformationen über ein angefordertes Spiel
...
Im Backend wird als Test-Framework „Pytest“~\cite{pytest} genutzt.


\subsection{Frontend}

Im Frontend wird als Test-Framework Vitest in Kombination mit Svelte Testing Library verwendet.
