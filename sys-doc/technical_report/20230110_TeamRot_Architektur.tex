...
Des Weiteren gibt Abbildung~\ref{fig:architecture} einen Überblick über die Architektur.
...
Außerdem werden einige numerische Werte wie das Veröffentlichungsdatum, das im Unix-Zeitstempel\footnote{Der Unix-Zeitstempel ist auch als „The Epoch“ bekannt und zählt die
Anzahl der vergangenen Sekunden seit 1. Januar 1970~\cite{unix}.} angegeben ist und der Standort des Entwicklerunternehmens als dreistelliger Country-Code\footnote{Der Country-Code wurde von der International Organization for Standardization entwickelt
und im ISO-3166 publiziert~\cite{iso}.} definiert.
Im Skript \texttt{igdb.py} wurde eine Funktion \texttt{change\_format} implementiert, welche deb Unix-Zeitstempel in das Zeitformat \texttt{DD-MM-YYYY} und den ISO-Country-Code in Ländernamen konvertiert.
...
\begin{figure*}[ht]
    ...
    \caption{Überblick über die Architektur von SGDb. Die Architektur besteht aus drei Diensten: Das Frontend (Abschnitt~\ref{subsec:frontend}) bietet dem Nutzer eine grafische Bedienoberfläche, das Backend (Abschnitt~\ref{subsec:backend}) stellt die API bereit und die Datenbank (Abschnitt~\ref{subsec:datenbank}) speichert persistent die Daten.}
...
Zur Beschreibung der Videospiele werden Ontologien von „schema.org“~\cite{schemaorg} verwendet.
Damit das Erscheinungsdatum auch als Datum interpretierbar ist, wird \texttt{xsd:dateTime} aus „XML Schema“~\cite{xmlschema} benutzt.
Um Videospiele vom Typ \texttt{schema:VideoGame} zu deklarieren, wird das RDF-Schema \texttt{typeof} eingesetzt. %TODO: Warum eigentlich?
...
Sigma.js zeichnet Graphen mit WebGL. Dadurch lassen sich größere Graphen schneller zeichnen als mit Canvas- oder SVG-basierten Lösungen.
...
Für das Graphen-Layout wird der „ForceAtlas2“~\cite{forceatlas2} Algorithmus verwendet.
Vor dem Start von ForceAtlas2 muss die Startposition jedes Knotens festgelegt werden.
Daher müssen zwei Attribute namens $x$ und $y$ für alle Knoten des Graphen definiert werden.
...
Die Filterselektion wurde auf fünf mögliche Filtermöglichkeiten begrenzt, da eine uneingeschränkte Auswahl an Filterkombinationen in den allermeisten Fällen zu keinem sinnvollen Ergebnis geführt hat.
...
Die Komponente führt asynchron eine HTTP-Anfrage an die Route \texttt{/detail/{game\_name}} der Backend-API aus, um Details zu einem Spiel abzurufen und die Detailseite damit zu befüllen.
Die Detailseite enthält auch einige HTML-Metatags, die Informationen über das Spiel enthalten, die für Social Media-Plattformen wie Twitter relevant sind.
Die Metatags enthalten den Titel des Spiels, eine Beschreibung und ein Bild, das im Tweet angezeigt werden soll.
Die Daten für diese Metatags werden ebenfalls von der Backend-API abgerufen, wenn die Komponente gerendert wird.
Schließlich enthält die Komponente ein Hauptelement, in dem die eigentlichen Spielinformationen angezeigt werden.
Die Informationen umfassen ein Bild des Spiels, den Titel des Spiels und weitere Details wie die Plattformen, auf denen das Spiel verfügbar ist, und das Veröffentlichungsdatum.
...
Für die Bereitstellung wird der Asynchronous Server Gateway Interface (ASGI) Server „Uvicorn“~\cite{uvicorn} verwendet.
